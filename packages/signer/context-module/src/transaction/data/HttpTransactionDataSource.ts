...
export class HttpTransactionDataSource implements TransactionDataSource {
  ...
  > {
    try {
      ...
      const calldata = response.data?.[0];
      if (!calldata) {
        return Left(
          new Error(
  ...
  private getReference(
    ...
    if (param.type === "TOKEN_AMOUNT" && param.token !== undefined) {
      return {
        ...
        value_path: this.toGenericPath(param.token.binary_path),
      };
    } else if (param.type === "NFT") {
      return {
        type: ClearSignContextType.NFT,
        value_path: this.toGenericPath(param.collection.binary_path),
      };
    } else if (param.type === "TRUSTED_NAME") {
      return {
        type: ClearSignContextType.TRUSTED_NAME,
        value_path: this.toGenericPath(param.value.binary_path),
        types: param.types,
  ...
  private toGenericPath(
    ...
    return path.elements.map((element) => {
      if (element.type === "ARRAY") {
        const { item_size: itemSize, ...rest } = element;
        return {
          itemSize,
          ...rest,
        };
