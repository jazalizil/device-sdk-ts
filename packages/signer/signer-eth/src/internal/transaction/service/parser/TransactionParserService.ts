...
export class TransactionParserService {
        ...
        case DataPathElementType.ARRAY:
          ...
          return this.getU256(data.slice(offset))
            ...
            .chain((length) => {
              ...
              offset += CHUNK_SIZE;
              parentOffset = offset;
              // Iterate on the whole array elements if offset is null
              if (element.offset === undefined) {
                return Array.from({ length }, (_, i) => i)
                  .map((i) => offset + i * element.itemSize * CHUNK_SIZE)
                  .reduce(
              ...
              // Else, use the given offset
              if (element.offset >= length || element.offset < -length) {
                return Left(
                  new Error(`Array index ${element.offset} out of bounds`),
                );
              }
              offset +=
                element.offset < 0
                  ? (length + element.offset) * element.itemSize * CHUNK_SIZE
                  : element.offset * element.itemSize * CHUNK_SIZE;
              return this.extractCalldataValueAt(
                data,
                path,
                offset,
                parentOffset,
  ...
  private sliceLeaf(
    ...
    const start =
      leafSlice.start >= 0 ? leafSlice.start : length + 1 + leafSlice.start;
    const end = leafSlice.end >= 0 ? leafSlice.end : length + 1 + leafSlice.end;
    if (start >= end) {
